...
const Footer: React.FC<FooterProps> = ({
  ...
  return (
    <footer
      ref={footerRef}
      className="bg-[#fff9f5] text-black relative flex flex-col w-full h-full justify-between lg:h-screen select-none border-t border-orange-200"
    >
        ...
        <div className="space-y-2">
          <ul className="flex flex-wrap gap-4">
            {leftLinks.map((link, index) => (
              <li key={index}>
                <a href={link.href} className="text-sm hover:text-orange-700">
                  {link.label}
                </a>
              </li>
            ))}
